            ...
            uploadTask.on(
                ...
                () => {
                    getDownloadURL(uploadTask.snapshot.ref).then(downloadURL => {
                        console.log("... resolve download Url:", downloadURL)
                        resolve(downloadURL);
                    })
                }
            )
      ...
      <main className="p-3 max-w-4xl max-auto">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
            <div className="flex flex-col gap-4 flex-1">
                ...
                <div className="flex gap-6 flex-wrap">
                    <div className="flex gap-2">
                        ...
                        checked={formData.type === 'sale'} onChange={handleChange} />
                        <span>Sell</span>
                    </div>
                    <div className="flex gap-2">
                        <input type="checkbox" id="rent" className="w-5"
                        checked={formData.type === 'rent'} onChange={handleChange} />
                        <span>Rent</span>
                    </div>
                    <div className="flex gap-2">
                        <input type="checkbox" id="parking" className="w-5"
                        checked={formData.parking} onChange={handleChange} />
                        <span>Parking spot</span>
                    </div>
                    <div className="flex gap-2">
                        <input type="checkbox" id="furnished" className="w-5"
                        checked={formData.furnished} onChange={handleChange} />
                        <span>Furnished</span>
                    </div>
                    <div className="flex gap-2">
                        <input type="checkbox" id="offer" className="w-5"
                        checked={formData.offer} onChange={handleChange} />
                        <span>Offer</span>
                    </div>
                </div>
            ...
            <div className="flex flex-col gap-4 flex-1">
                ...
                {error && <p className="p-1 text-red-600"> {error}</p>}
            </div>
        </form>
      </main> 
    )
